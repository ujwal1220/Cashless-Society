...
else
{
	...
	if(crdrs.next()==true)
	{
		 credit_taken=crdrs.getInt(9);
		 a_no=crdrs.getString(1);
		 crn=crdrs.getString(2);
				...
				if(credit_taken<=currentbalance)
				{
						...
						/* Updating the Credit Payment into Credit Card and Account Balance into Bank Account */
							...
							String sql31="insert into transactions(user,bank,amount,a_no,crn,date,status,tranfor,frombank,fromaccount_no,fromuser,balance_credited,credit_status)values('"+user+"','"+bank+"','"+amount+"','"+a_no+"','"+crn+"','"+date+"','"+"Success"+"','"+"Transfer"+"','"+bank+"','"+accno+"','"+user+"','"+amount+"','"+"Payment Done"+"')";
						Statement stmt31 = connection.createStatement();
						stmt31.executeUpdate(sql31);
						
							
					String nuser="insert into payment_fraud(user,bank,payment_cash,activity,date)values('"+user+"','"+bank+"','"+amount+"','"+"Normal"+"','"+date+"')";
				...
				String abuser="insert into payment_fraud(user,bank,payment_cash,activity,date)values('"+user+"','"+bank+"','"+"No Cash"+"','"+"Abnormal"+"','"+date+"')";
					Statement abstmt = connection.createStatement();
					abstmt.executeUpdate(abuser);
					
				String abuser1="insert into fraud(user,bank,amount,activity,site,date,fraud_type)values('"+user+"','"+bank+"','"+"No Cash"+"','"+"Abnormal"+"','"+"bank_transaction"+"','"+date+"','"+"payment_fraud"+"')";
